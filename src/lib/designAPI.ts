...
// Fallback using Unsplash Source (no API key required)
function generateFallbackImages(query: string, count: number): DesignAsset[] {
  const categories = ['business', 'technology', 'marketing', 'office', 'team', 'success', 'growth', 'innovation', 'strategy'];
  const relevantCategory = categories.find(cat => query.toLowerCase().includes(cat)) || 'business';
  
  return Array.from({ length: count }, (_, index) => {
    const imageId = 600 + index;
    return {
      id: `unsplash-${imageId}`,
      url: `https://source.unsplash.com/800x600/?${relevantCategory},${query}`,
      thumbnail: `https://source.unsplash.com/300x200/?${relevantCategory},${query}`,
      title: `${query} ${index + 1}`,
      description: `Professional ${query} image for business use`,
      category: relevantCategory,
      tags: [query, relevantCategory, 'business', 'professional']
    };
  });
}
